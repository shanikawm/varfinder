...
if [ -z $region ]; then
	...
	bcftools consensus -f $file -H A -s $sample "${pwd}/${sample}.vcf.gz" | sed "s/^>.*/>${sample}/" | bgzip > "${pwd}/${sample}.fasta.gz"
	samtools faidx "${pwd}/${sample}.fasta.gz"
else
	bcftools view -r $region --min-ac=1 -s $sample $vcf | bcftools norm -d all - | bgzip > "${pwd}/${sample}.vcf.gz"
	bcftools index "${pwd}/${sample}.vcf.gz"
	tabix "${pwd}/${sample}.vcf.gz"
	samtools faidx $file $region | bcftools consensus -H A -s $sample "${pwd}/${sample}.vcf.gz" | sed "s/^>.*/>${sample}/" | bgzip > "${pwd}/${sample}.fasta.gz"
	samtools faidx "${pwd}/${sample}.fasta.gz"
fi
